## This file runs MC simulations on just the DGP ##
## Data used is generated by Ox ##
## Data used is stored in case by case folders ##
...
cRep=1000
# set inclusion probability threshold 
prob <- 0.15
...
#delete x variables we don't want to include
	select <- c("Ly", "x1", "x2", "x3", "x4", "x5")
#put data into matrix form
	data_rep1a = data_rep1[select]
...
#create dataframe with true parameters
	...
	select1 <- c("Ly", "x1", "x2", "x3", "x4", "x5")
	truepardata = data_rep1[select1]
	regressors = paste("x", colnames(truepardata), sep = '')
	truepar.df = data.frame(regressors,truepar[c(2:7),])
...
for (i in 2:cRep)
{
	...
	select2 <- c("Ly", "x1", "x2", "x3", "x4", "x5")
	data_repi_a = data_repi[select2]
...
#Calculate total MSE for retained irrelevant varibles
	df.UMSE$RIVMSE <- matrix(0, nrow(df.UMSE),1)
	for(j in 1:nrow(df.UMSE))
	{
		if (df.UMSE$TrueParameterValues[j] == 0 )
		{
   		df.UMSE$RIVMSE[j] <- df.UMSE$RUMSE[j]
 		}
	}

	RMSE_retirrvar <- colSums(df.UMSE$RIVMSE)
...
#Select only those observations for potency calc
	...
	print(mean(df.gaugeind$gaugecalc))
 	print("total RMSE across retained irrelevant variables is")
 	print(RMSE_retirrvar)
